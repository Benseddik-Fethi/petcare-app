...
import {clearRefreshTokenCookie, REFRESH_TOKEN_COOKIE_NAME, setRefreshTokenCookie} from "../utils/security/cookies";
import {z} from "zod";
...
export class AuthController {
    ...
    static async social(req: Request, res: Response, next: NextFunction) {
        try {
            const {token} = socialSchema.parse(req.body);
    ...
    static async refresh(req: Request, res: Response, next: NextFunction) {
        try {
            ...
            if (!token) throw new UnauthorizedError("Session expirée");

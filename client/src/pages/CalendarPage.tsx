...
import {Calendar as CalendarIcon, ChevronLeft, ChevronRight, Clock, Dog, MapPin, User} from "lucide-react";
import {
    addMonths,
    eachDayOfInterval,
    endOfMonth,
...
export default function CalendarPage() {
    ...
    const renderAppointmentsList = () => {
        ...
        if (selectedDayAppointments.length === 0) {
            return (
                <div
                    className="text-center py-10 bg-gray-50 dark:bg-slate-950 rounded-2xl border-2 border-dashed border-gray-100 dark:border-slate-800">
        ...
        return (
            <div className="space-y-3">
                {selectedDayAppointments.map((apt) => (
                    <div key={apt.id}
                         className="p-4 rounded-2xl bg-gray-50 dark:bg-slate-950 border border-gray-100 dark:border-slate-800 hover:bg-rose-50 dark:hover:bg-slate-900 hover:border-rose-100 dark:hover:border-slate-700 transition-all group">
                        <div className="flex justify-between items-start mb-2">
              <span className={cn("text-xs font-bold px-2 py-1 rounded-md border", getTypeColor(apt.type))}>
                {apt.type}
              </span>
                            <span
                                className="text-sm font-bold text-gray-700 dark:text-gray-200">{format(new Date(apt.date), 'HH:mm')}</span>
                        </div>
        ...
        <div className="space-y-8 pb-10">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2">
                    <Card
                        className="p-6 border-white dark:border-slate-800 shadow-sm bg-white/80 dark:bg-slate-900/80 backdrop-blur-sm rounded-3xl">

                        {/* Navigation Mois */}
                        <div className="flex items-center justify-between mb-8">
                            <Button variant="ghost" onClick={() => setCurrentMonth(subMonths(currentMonth, 1))}
                                    className="hover:bg-rose-50 dark:hover:bg-slate-800 text-gray-600 dark:text-gray-300">
                                <ChevronLeft size={24}/>
                            </Button>
                        ...
                        <div className="grid grid-cols-7 mb-4 text-center">
                            {['Lun', 'Mar', 'Mer', 'Jeu', 'Ven', 'Sam', 'Dim'].map(day => (
                                <div key={day}
                                     className="text-sm font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wide">
                                    {day}
                                </div>
                            ))}
                        </div>
                    ...
                    <span className={cn(
                      ...
                      {format(day, 'd')}
                                        ...
                                        {dayAppts.slice(0, 2).map((apt) => (
                                            <div key={apt.id}
                                                 className={cn("text-[10px] font-medium px-2 py-1 rounded-lg truncate border w-full text-left", getTypeColor(apt.type))}>
                                        ...
                                        {dayAppts.length > 2 && (
                                            <div
                                                className="text-[10px] text-gray-400 dark:text-gray-500 pl-1">+{dayAppts.length - 2} autre(s)</div>
                                        )}
                                    </button>
                                );
                            })}
                ...
                <div className="space-y-6">
                    ...
                    {/* Détails du jour */}
                    <Card
                        className="p-6 border-white dark:border-slate-800 shadow-sm bg-white dark:bg-slate-900 rounded-3xl h-fit">
                    ...
                    {upcomingAppointments.length > 0 && (
                        <div className="pt-4">
                            <h4 className="text-sm font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-3">À
                                venir bientôt</h4>
                            <div className="space-y-3">
                                {upcomingAppointments.map((apt) => (
                                    <div key={apt.id}
                                         className="flex items-center gap-3 p-3 rounded-2xl bg-white/60 dark:bg-slate-900/60 border border-white dark:border-slate-800 shadow-sm hover:shadow-md dark:hover:shadow-none transition-all cursor-pointer">
                                        <div
                                            className="w-12 h-12 rounded-xl bg-gradient-to-br from-rose-100 to-pink-50 dark:from-rose-950 dark:to-rose-900 flex flex-col items-center justify-center text-rose-600 dark:text-rose-400 shrink-0">
                                            <span
                                                className="text-xs font-bold">{format(new Date(apt.date), 'dd')}</span>
                                            <span
                                                className="text-[9px] uppercase">{format(new Date(apt.date), 'MMM', {locale: fr})}</span>
                                        </div>
